...
export class PriceManager {
  ...
  constructor(opts: PriceManagerOpts) {
    this.truePrice = opts.truePrice;
  ...
  private priceWithinTolerance(actualPrice: number, forOutcome: boolean) {
    ...
    return prices.includes(actualPrice);
  }

  private findFirstPrice(
    truePrice: number,
    spread: number,
    forOutcome: boolean
  ) {
    ...
    if (forOutcome) return prices.slice(-1)[0];
    else return prices[0];
  }

  private pricesWithSteps(
    truePrice: number,
    spread: number,
    depth: number,
    steps: number,
    ...
    const prices = [];
    const trueSteps = steps + 1;
    const ladderLength = depth * trueSteps;
    const firstPrice = this.findFirstPrice(truePrice, spread, forOutcome);
    let fullArray = [];
    if (forOutcome) {
      fullArray = nItemsUpArray(firstPrice, ladderLength, DEFAULT_PRICE_LADDER);
    } else {
      fullArray = nItemsDownArray(
        firstPrice,
        ladderLength,
        DEFAULT_PRICE_LADDER
      );
    }
    for (const index of Array(depth).keys()) {
      prices.push(fullArray[index * trueSteps]);
    }
    return prices;
  }
}
